...
const Index = () => {
  ...
  // Controla o scroll do corpo da página
  useEffect(() => {
    if (!isRevealed) {
  ...
  const handleVideoLoad = () => {
    ...
    // Tenta dar play assim que carregar, mesmo antes do reveal
    if (videoRef.current) {
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-amber-100 overflow-x-hidden">
      ...
      {/* Conteúdo Principal - Sempre renderizado para forçar o carregamento do vídeo */}
      <main 
        className={`mx-auto bg-white min-h-screen shadow-2xl relative z-0 transition-opacity duration-1000 ${isRevealed ? 'opacity-100' : 'opacity-0'}`}
      ...
      >
        ...
        {/* SEÇÃO 2: Só aparece após o reveal */}
        {isRevealed && (
          <section id="section-2" className="w-full relative leading-[0] animate-in fade-in duration-1000">
            <img 
              src="/02.png" 
              alt="Convite Parte 2" 
              className="w-full h-auto block"
